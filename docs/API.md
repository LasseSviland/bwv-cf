...
The statistics endpoint counts fixed-assortment wine-store placements, so the same wine sold out at five stores counts as five stockouts. Optional local products are never inferred to be sold out when they are absent. A fixed placement starts on the product's current assortment-valid date (falling back to its introduction date when necessary), which prevents a product from being counted before it existed. `daily` reports zero-stock placements, distinct affected wines and stores, new positive-to-zero transitions, the last observed bottles before those transitions, and total portfolio bottles still in stock. `wines` reports every affected product, its fixed-store count and availability, exact sold-out dates, stores sold out per date, and period totals. `summary` reports distinct placements, wines, and stores affected; stockout days and placement-days; transition and bottle totals; average daily stockouts; placement availability; and the peak date. `bottlesLostToStockouts` describes inventory movement into zero stock and must not be interpreted as confirmed sales.
...
Wine summaries expose `assortment` and `assortmentGrades`; store summaries expose `monopolyCategory`, `monopolyProfile`, and `storeAssortment`. The statistics API and frontend combine these fields to reserve sold-out status for products assigned to a store's fixed assortment. Products from a higher category, a different demand profile, or an optional range remain additional local stock and are excluded from sold-out counts.
